...
- **수용 기준**:
  - [x] 이미 워치리스트에 있는 종목을 자동완성에서 다시 선택 → 확인 팝업이 나타나고 워치리스트 항목 수는 늘지 않는다
  - [x] 팝업에서 "확인" 클릭 → 해당 종목의 표시된 시세·점수가 다시 조회되어 갱신된다
  - [x] 팝업에서 "취소" 클릭 → 워치리스트 내용이 이전과 동일하게 유지된다
- **검증**: `bun run test -- duplicate-dialog`

---

### Checkpoint: Task 5-7 이후 ✅
- [x] 모든 테스트 통과: `bun run test` (44 tests)
- [x] 빌드 성공: `bun run build`
- [x] 검색 → 추가 → 중복 추가 시도 → 목록 표시까지 전체 흐름 — RTL 통합 테스트로 커버(userEvent 기반 실제 DOM 상호작용). 실제 브라우저(claude-in-chrome 확장 미연결)로는 확인 못함 — curl로 `/` 200 응답만 확인. 사용자가 `bun run dev`로 직접 눈으로 확인하는 걸 권장
...
- **수용 기준**:
  - [x] 종목이 담긴 상태에서 페이지 새로고침 → 이전에 추가된 종목명이 모두 그대로 나타난다
  - [x] 새로고침 시점에 가격이 달라졌다면 → 갱신된 가격이 화면에 표시된다 (mock 응답 값 변경으로 검증)
- **검증**: `bun run test -- use-watchlist`

---

### Checkpoint: 전체 완료 ✅
- [x] 모든 테스트 통과: `bun run test` (55 tests)
- [x] 빌드 성공: `bun run build`
- [x] Human review: `bun run dev`로 실행 후 `/` 200 응답 및 타이틀·헤딩("그레이엄 워치리스트"/"워치리스트") 확인. 브라우저 확장 미연결로 검색→추가→상세→삭제→새로고침의 실제 클릭 확인은 RTL 통합 테스트로 대체됨 — **사용자가 `bun run dev`로 직접 모바일/데스크톱 뷰포트 양쪽에서 눈으로 확인하는 것을 권장**
